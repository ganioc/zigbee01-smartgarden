...
static ZStatus_t zclGeneral_ProcessInSmartGarden(zclIncoming_t *pInMsg,
...
ZStatus_t zclGeneral_RegisterCmdCallbacks( uint8 endpoint, zclGeneral_AppCallbacks_t *callbacks )
{
  ...
  if ( zclGenPluginRegisted == FALSE )
  {
    ...
    debug_str("zclGenPluginRegisted");
...
static ZStatus_t zclGeneral_HdlIncoming( zclIncoming_t *pInMsg )
{
  ...
  debug_str("zclGeneral_HdlIncoming");
...
debug_str("zclGeneral_HdlInSpecificCommands");
  ...
  switch ( pInMsg->msg->clusterId )
  {
    ...
    case ZCL_CLUSTER_ID_GEN_ON_OFF:
      ...
      break;
        ...
        // Added by Yang
        case ZCL_CLUSTER_ID_SMARTGARDEN:
            stat = zclGeneral_ProcessInSmartGarden(pInMsg, pCBs);
            break;
        ...
        zclGeneral_AppCallbacks_t *pCBs)
{
    ...
    if(zcl_ServerCmd(pInMsg->hdr.fc.direction))
    {
        switch(pInMsg->hdr.commandID)
        {
            case COMMAND_IDENTIFYING:
            case COMMAND_TURN_ON_IRRIGATE:
            case COMMAND_TURN_OFF_IRRIGATE:
                if(pCBs->pfnSmartGarden)
                {
                    pCBs->pfnSmartGarden(pInMsg->hdr.commandID);
                }
                break;
            default:
              stat = ZFailure;
                                            ...
                                            zclGeneral_AppCallbacks_t *pCBs )
{
  ...
  if ( zcl_ServerCmd( pInMsg->hdr.fc.direction ) )
  {
    switch ( pInMsg->hdr.commandID )
    {
      ...
      case  COMMAND_TURN_ON_IRRIGATE:
      case  COMMAND_TURN_OFF_IRRIGATE:
      case COMMAND_TURN_ON_PERMIT_JOINING:   
        if ( pCBs->pfnOnOff )
        {
