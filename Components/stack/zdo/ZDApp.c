...
UINT16 ZDApp_event_loop( uint8 task_id, UINT16 events )
{
  ...
  {
    debug_str("ZDApp_event_loop ZSTACK_ROUTER_BUILD");
    if ( events & ZDO_NETWORK_START )
    {
      debug_str("ZDO_NETWORK_START");
    ...
    {
      debug_str("ZDO_PARENT_ANNCE_EVT");
  ...
  if ( events & ZDO_STATE_CHANGE_EVT )
  {
    ...
    cust_debug_str("ZDO_STATE_CHANGE_EVT:%d ", devState);
...
void ZDApp_NetworkStartEvt( void )
{
  if ( nwkStatus == ZSuccess )
  {
    ...
    if ( devState == DEV_COORD_STARTING )
    {
      ...
      HalLedBlink (HAL_LED_2, 60, 50, 2000);
...
void ZDO_JoinConfirmCB( uint16 PanId, ZStatus_t Status )
{
  ...
  if ( Status == ZSUCCESS )
  {
	...
	// Added by Yang
    cust_debug_str("Device Joined: %d" , PanId);
