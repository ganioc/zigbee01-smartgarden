...
{
  uint8 nvItemNum = 0;
  uint8 nvTotalItem = sizeof(zgItemTable) / sizeof(zgItem_t);
  for(; nvItemNum < nvTotalItem; nvItemNum++)
  {
    osal_nv_delete(zgItemTable[nvItemNum].id, zgItemTable[nvItemNum].len);
  }
}
